...
Com a adesão do modelo relacional, as empresas começaram a ter grandes volumes de dados e com o aumento da competitividade, era necessário transformar esses dados em informação. Como o modelo relacional não conseguia prover os mecanismos para análise gerencial dos dados, surgiram as Data Warehouses.

Por ser objeto de desejo de muitas empresas, soluções proprietárias foram desenvolvidas pelas mais diversas empresas de software. Foi a crescente demanda por ferramentas de Business Intelligence (BI) que possibilitou o surgimento de ferramentas Livres mais robustas. Apesar disso, ainda são muito poucas as ferramentas open-source que podem ser comparadas às proprietárias.

Três ferramentas foram apresentadas: O Mondrian, parte do projeto Pentaho da Hitachi, tem seu uso livre, no entanto possui módulos que são pagos. O SpagoBI que é uma solução totalmente open-source que unifica diversas ferramentas diferentes de forma a prover soluções de BI e o Vertica, software que teve seu desenvolvimento iniciado de forma open-source e que foi adquirido pela HP. O Vertica é uma solução completa para análise gerencial e possui uma versão community que permite o uso de todas as ferramentas contidas no software pago com limitações de tamanho da base e número de clusters.
